    ...
    func floorNumbers() {
        ...
        if !doorsOpened {
            ...
            if goingUp {
                ...
                ///we need to use similar code for the when the goingUo variable is not true, so both states, true and false, will operate the lights and sounds correctly
                ///so if not goingUp, keep the floor 2 light on for 5 seconds until the elevator doors close, and then turn it off and turn on the floor 1 lights, and play the chimes and door sound files.
            } else if !goingUp {
                withAnimation(Animation.default.delay(5.0)) {
                    floor1 = true
                    floor2 = false
